...
loadBalancerConfig
    :
      LOADBALANCERLABEL NEWLINE+
        algorithmStatement NEWLINE+
        persistenceStatement NEWLINE+
        sSLStatement NEWLINE+
        healthCheckStatement NEWLINE+
      ENDLOADBALANCERLABEL
    ;

//Definition algorithm statement.
algorithmStatement
    : ALGORITHMLABEL NEWLINE*
        NAME WS* COLON WS* ALGORITHMTYPEX NEWLINE+
        (UPDATEINTERVAL WS* COLON WS* LBTIMESTRINGX NEWLINE+)?
      ENDALGORITHMLABEL
    ;

//Definition persistence statement.
persistenceStatement
    : PERSISTENCELABEL NEWLINE+
        TYPE WS* COLON WS* PERSISTENCETYPEX NEWLINE+
        (SESSIONTIMEOUT WS* COLON WS* LBTIMESTRINGX NEWLINE+)?
      ENDPERSISTENCELABEL
    ;

//Definition SSL Statement.
sSLStatement
    : SSLSUPPORTLABEL NEWLINE+
        TYPE WS* COLON WS* SSLSUPPORTTYPEX NEWLINE+
       ENDSSLSUPPORTLABEL
    ;

//Definition health check statement.
healthCheckStatement
    : HEALTHCHECKLABEL NEWLINE+
        TYPE WS* COLON WS* HEALTHCHECKTYPEX NEWLINE+
        UNHEALTHYRETRIES WS* COLON WS* HEALTHCHECKRETRIESCOUNTX NEWLINE+
        HEALTHYRETRIES WS* COLON WS* HEALTHCHECKRETRIESCOUNTX NEWLINE+
        HEALTHYCHECKINTERVAL WS* COLON* WS* LBTIMESTRINGX NEWLINE+
      ENDHEALTHCHECKLABEL
    ;
