...
loadBalancerConfig
    :
      LOADBALANCERX NEWLINE+
        algorithmStatement NEWLINE+
        persistenceStatement NEWLINE+
        sSLStatement NEWLINE+
        healthCheckStatement NEWLINE+
      ENDLOADBALANCERCONFIGX
    ;

algorithmStatement
    : ALGORITHMX NEWLINE*
        NAME WS* COLON WS* ALGORITHMTYPEX NEWLINE+
        (UPDATEINTERVAL WS* COLON WS* TIMESTRINGX NEWLINE+)?
      ENDALGORITHMX
    ;

persistenceStatement
    : PERSISTENCEX NEWLINE+
        TYPELABEL WS* COLON WS* PERSISTENCETYPEX NEWLINE+
        (SESSIONTIMEOUT WS* COLON WS* TIMESTRINGX NEWLINE+)?
      ENDPERSISTENCEX
    ;

sSLStatement
    : SSLSUPPORTX NEWLINE+
        TYPELABEL WS* COLON WS* SSLSUPPORTTYPEX NEWLINE+
       ENDSSLSUPPORTX
    ;

healthCheckStatement
    : HEALTHCHECKX NEWLINE+
        UNHEALTHYRETRIESLABEL WS* COLON WS* RETRIESCOUNTX NEWLINE+
        HEALTHYRETRIESLABEL WS* COLON WS* RETRIESCOUNTX NEWLINE+
        HEALTHYCHECKINTERVAL WS* COLON* WS* TIMESTRINGX NEWLINE+
      ENDHEALTHCHECKX
    ;
...
// LEXER: LB Keywords
ENDLOADBALANCERCONFIGX : ENDLOADBALANCERCONFIG;
ENDALGORITHMX : ENDALGORITHM;
ENDPERSISTENCEX : ENDPERSISTENCE;
LOADBALANCERX : LOADBALANCER;
ALGORITHMX : ALGORITHM;
PERSISTENCEX : PERSISTENCE;
ALGORITHMTYPEX : ALGORITHMTYPE;
UPDATEINTERVAL: U P D A T E I N T E R V A L;
TIMESTRINGX: TIMESTRING;
PERSISTENCETYPEX: PERSISTENCETYPE;
TYPELABEL: T Y P E;
SESSIONTIMEOUT: S E S S I O N T I M E O U T;
SSLSUPPORTX: SSLSUPPORT;
ENDSSLSUPPORTX: ENDSSLSUPPORT;
SSLSUPPORTTYPEX: SSLSUPPORTTYPE;
HEALTHCHECKX: HEALTHCHECK;
ENDHEALTHCHECKX: ENDHEALTHCHECK;
UNHEALTHYRETRIESLABEL: U N H E A L T H Y R E T R I E S;
HEALTHYRETRIESLABEL:  H E A L T H Y R E T R I E S;
HEALTHYCHECKINTERVAL: H E A L T H Y C H E C K I N T E R V A L;
RETRIESCOUNTX: RETRIESCOUNT;
...
fragment ENDLOADBALANCERCONFIG: E N D L O A D B A L A N C E R;
fragment ENDALGORITHM: E N D A L G O R I T H M;
fragment ENDPERSISTENCE: E N D P E R S I S T E N C E;
fragment LOADBALANCER: L O A D B A L A N C E R;
fragment ALGORITHM: A L G O R I T H M;
fragment PERSISTENCE: P E R S I S T E N C E;
fragment SSLSUPPORT: S S L;
fragment ENDSSLSUPPORT: E N D S S L;
fragment HEALTHCHECK: H E A L T H C H E C K;
fragment ENDHEALTHCHECK: E N D H E A L T H C H E C K;
...
fragment TIMESTRING: (DIGIT)+ ('h' | 'm' | 's' | 'ms');
fragment RETRIESCOUNT: (DIGIT | '10')+;
