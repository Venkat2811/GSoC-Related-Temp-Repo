...
//Definition load balancer config.
loadBalancerConfig
    :
      LOADBALANCERLABEL (WHITESPACE | WS | NEWLINE)*
      LPAREN (WHITESPACE | WS | NEWLINE)*
        algorithmStatement (WHITESPACE | WS | NEWLINE)* COMMA_SYMBOL (WHITESPACE | WS | NEWLINE)*
        persistenceStatement (WHITESPACE | WS | NEWLINE)* COMMA_SYMBOL (WHITESPACE | WS | NEWLINE)*
        sSLStatement (WHITESPACE | WS | NEWLINE)* COMMA_SYMBOL (WHITESPACE | WS | NEWLINE)*
        healthCheckStatement (WHITESPACE | WS | NEWLINE)*
      RPAREN
    ;

//Definition algorithm statement.
algorithmStatement
    : ALGORITHMLABEL (WHITESPACE | WS | NEWLINE)*
      LPAREN (WHITESPACE | WS | NEWLINE)*
        ALGORITHMNAMEDEF (WHITESPACE | WS | NEWLINE)*
        OTHERALGORITHMPARAMS* (WHITESPACE | WS | NEWLINE)*
      RPAREN
    ;

//Definition persistence statement.
persistenceStatement
    : PERSISTENCELABEL (WHITESPACE | WS | NEWLINE)*
      LPAREN (WHITESPACE | WS | NEWLINE)*
        PERSISTENCETYPEDEF (WHITESPACE | WS | NEWLINE)*
        OTHERPERSISTENCEPARAMS* (WHITESPACE | WS | NEWLINE)*
      RPAREN
    ;

//Definition SSL Statement.
sSLStatement
    : SSLSUPPORTLABEL (WHITESPACE | WS | NEWLINE)*
      LPAREN (WHITESPACE | WS | NEWLINE)*
        SSLTYPEDEF (WHITESPACE | WS | NEWLINE)*
      RPAREN
    ;

//Definition health check statement.
...
ALGORITHMNAMEDEF: NAME (WHITESPACE | WS | NEWLINE)*
                  LPAREN (WHITESPACE | WS | NEWLINE)*
                    ALGORITHMTYPEX (WHITESPACE | WS | NEWLINE)*
                  RPAREN
                ;

OTHERALGORITHMPARAMS: (WHITESPACE | WS | NEWLINE)* COMMA_SYMBOL (WHITESPACE | WS | NEWLINE)*
                      UPDATEINTERVAL (WHITESPACE | WS | NEWLINE)*
                      LPAREN (WHITESPACE | WS | NEWLINE)*
                        LBTIMESTRINGX (WHITESPACE | WS | NEWLINE)*
                      RPAREN
                    ;

PERSISTENCETYPEDEF: TYPE (WHITESPACE | WS | NEWLINE)*
                    LPAREN (WHITESPACE | WS | NEWLINE)*
                      PERSISTENCETYPEX (WHITESPACE | WS | NEWLINE)*
                    RPAREN
                  ;

OTHERPERSISTENCEPARAMS: (WHITESPACE | WS | NEWLINE)* COMMA_SYMBOL (WHITESPACE | WS | NEWLINE)*
                         SESSIONTIMEOUT (WHITESPACE | WS | NEWLINE)*
                         LPAREN (WHITESPACE | WS | NEWLINE)*
                            LBTIMESTRINGX (WHITESPACE | WS | NEWLINE)*
                         RPAREN
                      ;

SSLTYPEDEF: TYPE (WHITESPACE | WS | NEWLINE)*
            LPAREN (WHITESPACE | WS | NEWLINE)*
              SSLSUPPORTTYPEX (WHITESPACE | WS | NEWLINE)*
            RPAREN
          ;

HEALTHCHECKTYPEDEF: TYPE (WHITESPACE | WS | NEWLINE)*
                    LPAREN (WHITESPACE | WS | NEWLINE)*
                      HEALTHCHECKTYPEX (WHITESPACE | WS | NEWLINE)*
                    RPAREN
                  ;

OTHERHEALTHCHECKPARAMS: (WHITESPACE | WS | NEWLINE)* COMMA_SYMBOL (WHITESPACE | WS | NEWLINE)*
                        (UNHEALTHYRETRIES | HEALTHYRETRIES | HEALTHYCHECKINTERVAL) (WHITESPACE | WS | NEWLINE)*
                        LPAREN (WHITESPACE | WS | NEWLINE)*
                            (HEALTHCHECKRETRIESCOUNTX | LBTIMESTRINGX) (WHITESPACE | WS | NEWLINE)*
                        RPAREN
                     ;
...
ENDLOADBALANCERLABEL: E N D L O A D B A L A N C E R;
ALGORITHMLABEL: A L G O R I T H M;
ENDALGORITHMLABEL: E N D A L G O R I T H M;
PERSISTENCELABEL: P E R S I S T E N C E;
ENDPERSISTENCELABEL: E N D P E R S I S T E N C E;
SSLSUPPORTLABEL: S S L;
ENDSSLSUPPORTLABEL: E N D S S L;
HEALTHCHECKLABEL: H E A L T H C H E C K;
ENDHEALTHCHECKLABEL: E N D H E A L T H C H E C K;

ALGORITHMTYPEX : ALGORITHMTYPE;
PERSISTENCETYPEX: PERSISTENCETYPE;
SSLSUPPORTTYPEX: SSLSUPPORTTYPE;
HEALTHCHECKTYPEX: HEALTHCHECKTYPE;

UPDATEINTERVAL: U P D A T E I N T E R V A L;
TYPE: T Y P E;
SESSIONTIMEOUT: S E S S I O N T I M E O U T;
UNHEALTHYRETRIES: U N H E A L T H Y R E T R I E S;
HEALTHYRETRIES:  H E A L T H Y R E T R I E S;
HEALTHYCHECKINTERVAL: H E A L T H Y C H E C K I N T E R V A L;
